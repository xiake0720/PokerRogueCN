...
func _run() -> void:
	...
	_expect(consumable_source.contains("run.consumables.size()"), "ConsumableTray does not size itself from owned cards")
	_expect(not consumable_source.contains("EmptySlotOverlay"), "ConsumableTray still renders empty placeholders")
	...
	_expect(consumable_scene_source.contains("RegionOutline"), "ConsumableTray is missing its simple region outline")
	...
	var consumable_slot_source := FileAccess.get_file_as_string("res://scenes/ui/shared/consumable_slot_view.tscn")
	_expect(consumable_slot_source.contains("StyleBoxEmpty"), "Consumable cards still have individual slot frames")
	_expect(not consumable_slot_source.contains("EmptySlotOverlay"), "Consumable card view still contains an empty placeholder")
	...
	_expect(joker_scene_source.contains("RegionOutline"), "JokerShelf is missing its simple region outline")
	var joker_script_source := FileAccess.get_file_as_string("res://scripts/game/table/joker_shelf.gd")
	_expect(joker_script_source.contains("run.jokers.size()"), "JokerShelf does not size itself from owned jokers")
	_expect(not joker_script_source.contains("maxi(5, run.joker_slots)"), "JokerShelf still reserves empty placeholders")
